...
(quote "do you know the way")
; expect "do you know the way"
...
(null? (begin 1 0 1 0 nil))
; expect #t

(lambda (x) (cons x y))
; expect (lambda (x) (cons x y))

(lambda (a b c d e f) (list (a b c d e f)))
; expect (lambda (a b c d e f) (list (a b c d e f)))

(define (f a b c) (* 10 10))
f
; expect (lambda (a b c) (* 10 10))
...
(define (f a b c) (+ 2 a))
f
; expect (lambda (a b c) (+ 2 a))

global_frame = create_global_frame()
frame = global_frame.make_child_frame(Pair('x', Pair('y', nil)), Pair(1, nil))
; expect Error

frame = global_frame.make_child_frame(Pair('1', Pair('2', nil)), Pair('x', Pair('y', nil)))
; expect Error

(define (multiply-three x) (* x 3))
(multiply-three 3)
; expect 9
...
(define (add-front y x) (cons y x))
(add-front 4 (list 3 2 1))
; expect (4 3 2 1)

(define (f x) (define (g y) (define (h z) (+ x y z)) (h 10)) (g 10))
(f 10)
; expect 30
...
(or (odd? 4) (odd? 3) (odd? 2))
; expect #t

(define x 4)
(cond
  ((= 4 3) (define (func x) (+ x 3)))
  ((< 4 4) (define (func x) (+ x x)))
  ((= x 4) (define x 5))
  (else 0)
)
; expect x
...
(let ((a 1) (b 2) (a 3)) (* a b c))
; expect Error

(let ((a (+ 0 1)) (b (/ 4 2)) (c (car (list 3 2 1)))) (* a b c))
; expect 6
...
(define (power base n)
  (define (helper total base n)
    (if (= n 0)
      total
      (helper (* total base) base (- n 1))
    )
  )
  (helper 1 base n)
)
...
(define (increments start n)
  (cond
    ((= n 0) (print start))
    ((even? start) (increments (+ start 1) (- n 1)))
    ((odd? start) (increments (+ start 2) (- n 1)))
    )
  )
(increments 0 34)
; expect 67

(define-macro (lambdamaker args body)
  (list 'lambda args body)
)
((lambdamaker (a b c d e f g) (+ a b c d e f g)) 1 2 3 4 5 6 7)
; expect 28

((lambdamaker (c d) ((lambdamaker (b) ((lambdamaker (a) (+ a b c d)) 1)) 2)) 3 4)
; expect 10
