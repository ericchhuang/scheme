...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    instance = scheme_eval(first, env)
    if isinstance(instance, MacroProcedure):
        result = instance.apply_macro(rest, env)
        return scheme_eval(result, env)
    else:
        # BEGIN PROBLEM 5
        args = rest.map(lambda x: scheme_eval(x, env))
        return scheme_apply(instance, args, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    if expressions is nil:
        return None
    results = expressions
    while results.second is not nil:
        scheme_eval(results.first, env)
        results = results.second
    return scheme_eval(results.first, env, True)
    # END PROBLEM 8
...
class Frame:
    ...
    def lookup(self, symbol):
        ...
        if self.parent == None:
            raise SchemeError('{0} not found'.format(symbol))
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            ...
            raise SchemeError('unable to apply function')
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        expr = Pair(expressions.first.second, expressions.second)
        env.define(target.first, do_lambda_form(expr, env))
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    bindings.map(lambda x: check_form(x, 1, 2))
    check_formals(formals)
...
def do_define_macro(expressions, env):
    ...
    if isinstance(target, Pair) and scheme_symbolp(target.first):
        ...
        raise SchemeError('improper macro')
    # END Problem 21
